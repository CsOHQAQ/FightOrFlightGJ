...
public class ConsumableItem : BaseItem, IStackable, IActivatable {
    ...
    public void HoldUse(IPlayerCharacter user, ActivationTrigger trigger) {
        // Consumables may not need to implement a hold logic; leave empty if not required.
    }

    public void EndUse(IPlayerCharacter user, ActivationTrigger trigger) {
        // No special logic required for ending usage.
    }

    public void OnScroll(IPlayerCharacter user, float scrollDelta) {
